  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Container(
        ...
        child: FlashcardData.flashcards.isEmpty
            ? const Center(child: Text('No flashcards available.', style: TextStyle(color: AppColors.textColor)))
            : ListView.builder(
                padding: const EdgeInsets.all(16).copyWith(bottom: 80),
                itemCount: FlashcardData.flashcards.length,
                itemBuilder: (context, index) {
                  final flashcard = FlashcardData.flashcards[index];
                  if (!flashcard.containsKey('question') || !flashcard.containsKey('correctAnswer')) {
                    return const Center(child: Text('Invalid flashcard data.', style: TextStyle(color: AppColors.textColor)));
                  }
                  return Card(
                    elevation: 2,
                    margin: const EdgeInsets.symmetric(vertical: 8),
                    child: ListTile(
                      title: Text(flashcard['question']!, style: const TextStyle(color: AppColors.textColor)),
                      subtitle: Text(flashcard['correctAnswer']!, style: const TextStyle(color: AppColors.textColor)), 
                      trailing: Row(
